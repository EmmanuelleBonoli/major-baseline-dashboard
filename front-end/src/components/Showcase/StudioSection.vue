  ...
  <section class="relative py-20 md:py-32 px-4 md:px-8 z-[2]" id="studio">
    <div class="max-w-[1200px] mx-auto text-center">
      ...
      >
        ...
        ></div>
        <div
          class="ambient-glow-teal absolute bottom-0 left-0 w-48 h-48 rounded-full translate-y-1/2 -translate-x-1/2 blur-2xl"
        ></div>

        <div class="relative z-[3]">
          <h3
            class="text-teal text-[1.6rem] md:text-[2rem] font-bold tracking-[2px] mb-6 uppercase flex items-center gap-3"
          >
            <span class="text-gold text-[1.2rem]">/*</span> Ambition
          </h3>
        ...
        <div
          class="relative z-[3] flex flex-col items-center justify-center border-t md:border-t-0 md:border-l border-teal/20 pt-10 md:pt-0 md:pl-12"
        >
          <div class="w-32 h-32 md:w-40 md:h-40 relative flex items-center justify-center mb-6">
            <div
              class="absolute inset-0 border-2 border-dashed border-teal/50 rounded-full animate-[spin_10s_linear_infinite]"
            ></div>
            <div
...
.studio-card {
  ...
  box-shadow: 0 0 30px color-mix(in srgb, var(--color-teal) 15%, transparent);
}

.ambient-glow-gold {
  background-image: radial-gradient(circle, color-mix(in srgb, var(--color-gold) 10%, transparent), transparent);
}

.ambient-glow-teal {
  background-image: radial-gradient(circle, color-mix(in srgb, var(--color-teal) 15%, transparent), transparent);
}

.avatar-container {
  box-shadow: inset 0 0 20px color-mix(in srgb, var(--color-teal) 50%, transparent);
}
</style>
