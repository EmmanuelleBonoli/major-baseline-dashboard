<template>
  <section class="relative py-20 md:py-32 px-4 md:px-8 z-[2]" id="projects">
    <h2 class="section-title text-center text-[2.5rem] md:text-[4rem] mb-2 md:mb-4 font-black tracking-[3px] md:tracking-[5px] uppercase text-white">Nos Projets</h2>

    <div class="flex justify-center gap-4 mb-12 flex-wrap">
      <button 
        @click="filter = 'all'" 
        :class="['filter-btn px-6 py-2 border border-teal rounded-full text-sm font-bold tracking-[1px] uppercase transition-all cursor-pointer', filter === 'all' ? 'active' : 'text-teal']"
      >
        Tous
      </button>
      <button 
        @click="filter = 'app'" 
        :class="['filter-btn px-6 py-2 border border-teal rounded-full text-sm font-bold tracking-[1px] uppercase transition-all cursor-pointer', filter === 'app' ? 'active' : 'text-teal']"
      >
        Applications
      </button>
      <button 
        @click="filter = 'game'" 
        :class="['filter-btn px-6 py-2 border border-teal rounded-full text-sm font-bold tracking-[1px] uppercase transition-all cursor-pointer', filter === 'game' ? 'active' : 'text-teal']"
      >
        Jeux
      </button>
    </div>

    <div class="max-w-[1600px] mx-auto grid grid-cols-[repeat(auto-fit,minmax(min(100%,350px),1fr))] gap-8 md:gap-12 pl-4 pr-4">
