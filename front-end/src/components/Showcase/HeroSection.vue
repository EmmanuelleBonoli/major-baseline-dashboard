  ...
  <section class="relative mt-0 min-h-screen flex items-center justify-center z-[2]" id="home">
    ...
    <div class="max-w-[1400px] p-8 text-center z-[3] relative">
      ...
      >
        ...
        <a
          href="#studio"
          class="btn-glitch gold flex-1 min-w-[140px] max-w-[200px] md:max-w-none relative py-3 md:py-6 px-4 md:px-16 bg-transparent no-underline border-[2px] md:border-[3px] font-black text-[0.85rem] md:text-[1.1rem] tracking-[1px] md:tracking-[3px] uppercase overflow-hidden transition-all duration-300"
          >Le Studio</a
        >
      </div>
    </div>
...
<style scoped>
.hero-glow {
  background-image: radial-gradient(circle, color-mix(in srgb, var(--color-teal) 10%, transparent), transparent);
  animation: heroGlitch 5s infinite;
}

.hero-subtitle {
  text-shadow: 0 0 20px color-mix(in srgb, var(--color-teal) 50%, transparent);
}
...
.btn-glitch.teal::before {
  background: linear-gradient(90deg, transparent, var(--color-teal), transparent);
}
.btn-glitch.teal:hover {
  color: #fff;
  box-shadow:
    0 0 20px var(--color-teal),
    inset 0 0 20px color-mix(in srgb, var(--color-teal) 50%, transparent);
}
...
.btn-glitch.gold::before {
  background: linear-gradient(90deg, transparent, var(--color-gold), transparent);
}
.btn-glitch.gold:hover {
  color: #fff;
  box-shadow:
    0 0 20px var(--color-gold),
    inset 0 0 20px color-mix(in srgb, var(--color-gold) 50%, transparent);
}
...
.glitch-text::before {
  ...
  width: 100%;
  height: 100%;
  text-shadow: -2px 0 var(--color-teal);
