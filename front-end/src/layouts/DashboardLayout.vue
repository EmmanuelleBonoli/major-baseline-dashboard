<template>
  <div class="w-full min-h-screen flex flex-col md:flex-row bg-[#080d14]">
    ...
    <header class="md:hidden bg-black/80 backdrop-blur-[20px] border-b border-teal/30 shadow-glow-teal sticky top-0 z-[100] px-6 h-[70px] flex justify-between items-center">
      <div class="logo-section flex items-center gap-4">
        <router-link to="/dashboard" class="no-underline flex items-center gap-2">
          ...
          <h1 class="text-[1.2rem] font-bold text-teal shadow-teal [text-shadow:0_0_10px_var(--color-teal)]">Dashboard</h1>
        </router-link>
      </div>
    ...
    >
      <div class="hidden md:flex items-center justify-center h-[100px] px-8 border-b border-teal/10 shrink-0">
        <router-link to="/dashboard" class="no-underline w-full flex items-center justify-center gap-2 text-teal shadow-teal [text-shadow:0_0_10px_var(--color-teal)]">
      ...
      <nav class="flex-1 overflow-hidden py-6 px-4 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-2 mt-4 flex-1 overflow-hidden" v-if="applications.length > 0">
          ...
          >
            ...
            <component :is="ChevronDown" :size="16" class="transition-transform duration-300" :class="isProjectsExpanded ? 'rotate-180' : ''" />
          </button>
        ...
        <div class="mt-auto border-t border-teal/20 pt-8 flex flex-col gap-2">
          <router-link to="/" class="text-white/50 no-underline font-medium text-[0.95rem] transition-all duration-300 cursor-pointer flex items-center p-3 rounded-lg hover:text-white hover:bg-white/5">
            <component :is="Globe" :size="20" class="mr-3 opacity-70" /> Retour Vitrine
          </router-link>
          
          <a href="#" @click.prevent="logout" class="text-red-400 no-underline font-medium text-[0.95rem] transition-all duration-300 cursor-pointer flex items-center p-3 rounded-lg hover:text-white hover:bg-red-500/10 hover:border-red-500/30 border border-transparent">
    ...
    <!-- Main Content -->
    <main class="flex-1 min-w-0 w-full max-w-[1400px] mx-auto p-4 md:p-8 mt-4 md:mt-0 min-h-[calc(100vh-70px)] md:min-h-screen">
      <slot />
    </main>
  </div>
</template>
