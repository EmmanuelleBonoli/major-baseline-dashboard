...
public class DashboardService {
    ...
    public AppDashboardDTO getApplicationDashboard(UUID applicationId, LocalDate startDate, LocalDate endDate) {
        ...
        for (Store store : application.getStores()) {
            ...
            List<AppStats> storePrevStats = appStatsService.fetchAndGetStatsForDateRange(
                store.getId(),
                startDate.minusDays(diffDays),
            ...
            storeSummaries.add(
                new AppDashboardDTO.StoreSummary(store.getId(), storeSum.totalDownloads(), storeSum.totalRevenue(), storeSum.currency(), latestRating)
            );
        }
    ...
    private AppDashboardDTO.Summary calculateSummary(List<AppStats> current, List<AppStats> previous) {
        ...
        return new AppDashboardDTO.Summary(
            currentDownloads,
            calculateVariation(currentDownloads, prevDownloads),
            currentActiveUsers,
            calculateVariation(currentActiveUsers, prevActiveUsers),
            currentRevenue,
            calculateVariation(currentRevenue, prevRevenue),
            currency,
            currentCrashes,
            calculateVariation(currentCrashes, prevCrashes)
        );
    }
    ...
    private AppDashboardDTO.ChartData createChartData(String label, List<AppStats> stats, AppStats.MetricType metricType) {
        List<AppDashboardDTO.DataPoint> dataPoints = stats
            .stream()
